...
- `stash` 기능을 사용하기 위해선 변경 사항들이 트래킹이 되고 있는 상태여야 헙나다. 그렇기에 진행하기 전에 `git add`로 추가해줍니다.
- `git stash` 입력시 스테시라는 공간으로 변경사항이 이동합니다.
- `git stash pop`으로 스테시 공간의 내용을 불러옵니다.
...
| git stash apply | 치워둔 마지막 항목(번호 없을 시) 삭제         | 끝에 번호로 항목 지정 가능     |
| git stash apply | 치워둔 마지막 항목(번호 없을 시) 적용 및 삭제 | apply + drop                   |
| git stash apply | 새 브랜치를 생성하여 pop                      | 충돌사항이 있는 상황 등에 유용 |
| git stash apply | 치워둔 모든 항목들 비우기                     |                                |
...
[버전 달기 규칙은 여기를 참고하세요.](https://semver.org/lang/ko/)

- 태그 종류
  - `lightweight` 특정 커밋을 가리키는 용도
  - `annotated` 작성자 정보와 날짜, 메시지 GPG 서명 포함 기능

`git tag "v2.0.0(원하는 태그를 입력하세요)"`

- 로컬에 태그 달기

  - `-d` 태그 삭제 옵션
  - 옵션이 없으면 현재 존재하는 태그 보여주기
  - `-a` 어노테이티드 태그 생성
  - 원하는 커밋에 태그를 달고 싶다면 `git tag "태그명" "커밋 해시" -m "메세지 내용"`
...
다른 브런치와 merge할 때, 현재 브런치에서 변화된 내용이 없이, 다른 브런치의 내용만 가져온다면, 굳이 새로운 commit을 만들어 내지 않고 현재 브런치를 머지할 브런치의 커밋으로 이동사킵니다.

### 3 way merge

`git`이 현재 머지할 커밋들의 내용을 비교하기 위해, 분기된 `commit`과 양쪽의 커밋으로 비교하고 새로운 커밋을 생성하는 방식입니다.(즉 각각의 머지 브런치가 독립적인 commit을 소유하고 있습니다.)

### 체리픽

특정 브런치의 모든 커밋을 가져오는 것이 아니라, 특정 커밋만 가져오려면 `git cherry-pick (체리픽할 해시)`

### 잔가지만 가져오기

`git rebase --onto "도착 브런치" "출발 브런치" "이동할 브런치"`

### 협업을 위한 브런치 활용법

`gitflow`라는 활용법(깃 전략)이 가장 유명합니다.

[여기를 참조 하세요](https://nvie.com/posts/a-successful-git-branching-model/)
